...
class book_repository extends repository
{
    ...
    private ?\PDOStatement $findBooksByIdStatement = null;
    ...
    {
        if ($this->findBooksByIdStatement === null) {
            $this->findBooksByIdStatement = $this->pdo->prepare("SELECT * FROM {$this->table} WHERE id = :id");
        }

        $this->findBooksByIdStatement->execute([
            ":id" => $id
        ]);

        $row = $this->findBooksByIdStatement->fetch(\PDO::FETCH_ASSOC);
